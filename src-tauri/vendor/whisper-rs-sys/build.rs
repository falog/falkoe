...
fn main() {
    ...
    // Allow passing any WHISPER / GGML / CMAKE compile flags.
    //
    // In particular, enabling GGML_CPU_ALL_VARIANTS allows a single build to
    // ship multiple CPU backends (SSE/AVX/AVX2/etc) and pick the best one at
    // runtime based on the user's CPU features.
    for (key, value) in env::vars() {
        ...
        // Convenience: allow WHISPER_GGML_FOO=... as an alias for GGML_FOO=...
        // so users can keep all knobs under a WHISPER_* prefix in Cargo config.
        if let Some(rest) = key.strip_prefix("WHISPER_GGML_") {
            let ggml_key = format!("GGML_{rest}");
            config.define(&ggml_key, &value);
        }
    }
